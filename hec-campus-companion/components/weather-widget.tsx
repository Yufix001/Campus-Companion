    ...
    const fetchWeather = async () => {
      try {
        const res = await fetch(
          "https://api.open-meteo.com/v1/forecast?latitude=48.762&longitude=2.168&current=temperature_2m,weather_code&timezone=Europe%2FParis",
        );
  ...
  const getWeatherIcon = (code: number) => {
    if (code === 0 || code === 1)
      return (
        <Sun
          size={compact ? 18 : 20}
          className={compact ? "text-amber-500" : ""}
        />
      );
    if (code === 2)
      return (
    ...
    if (code === 3)
      return (
        <Cloud
          size={compact ? 18 : 20}
          className={compact ? "text-slate-400" : ""}
        />
      );
    if (code >= 45 && code <= 48)
      return (
    ...
    if (code >= 71 && code <= 77)
      return (
        <CloudSnow
          size={compact ? 18 : 20}
          className={compact ? "text-blue-200" : ""}
        />
      );
    if (code >= 80 && code <= 82)
      return (
        <CloudRain
          size={compact ? 18 : 20}
          className={compact ? "text-blue-500" : ""}
        />
      );
    if (code >= 95)
      return (
        <CloudLightning
          size={compact ? 18 : 20}
          className={compact ? "text-amber-600" : ""}
        />
      );
    return (
      <CloudSun
  ...
  if (loading) {
    return (
      <div
        className={
          compact
            ? "flex items-center gap-2 text-slate-500 text-sm opacity-50"
            : "flex flex-col justify-between h-full opacity-50"
        }
      >
        {!compact && <CloudSun size={20} />}
        <span className={compact ? "" : "mt-4 text-[10px]"}>
          {compact ? "Loading..." : "Loading Weather..."}
        </span>
      </div>
    );
  }
