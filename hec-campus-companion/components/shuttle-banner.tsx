...
export function ShuttleBanner() {
    const [nextShuttle, setNextShuttle] = useState<{ time: string, dest: string } | null>(null)
        ...
        <div className="w-full bg-[#07305B] text-white rounded-xl shadow-sm overflow-hidden flex flex-col sm:flex-row sm:items-center justify-between p-4 mb-6">
            <div className="flex items-center gap-4 mb-2 sm:mb-0">
                ...
                <div className="flex flex-col">
                    <span className="text-[10px] font-bold uppercase tracking-widest text-white/50">Next Shuttle</span>
                    <div className="flex items-baseline gap-2">
                        <span className="text-xl font-serif font-medium">{nextShuttle.time}</span>
                        <span className="text-sm text-white/80">• To {nextShuttle.dest}</span>
                    </div>
                </div>
            </div>
